...
check("Release scripts", ["test", "build", "production:check", "healthcheck", "domain:check", "firebase:domain-check", "hosting:security-check", "release:check", "secrets:plan", "database:smoke", "object:smoke", "runtime:smoke", "preaws:check", "internal:audit", "launch:verify", "launch:verify:live"].every((script) => packageJson.scripts?.[script]), "release scripts registered");
