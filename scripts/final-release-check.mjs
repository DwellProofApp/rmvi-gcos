...
check("Release scripts", ["test", "build", "production:check", "healthcheck", "domain:check", "release:check", "secrets:plan", "database:smoke", "object:smoke", "runtime:smoke", "launch:verify", "launch:verify:live"].every((script) => packageJson.scripts?.[script]), "release scripts registered");
